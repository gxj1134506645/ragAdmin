...
- 已完成主线：认证授权、用户管理、模型管理、文档处理管线（含图片处理 + 内容感知清洗）、混合检索（默认 HYBRID）、查询改写（Multi-Query + HyDE）、RAG SSE 聊天、会话记忆三层架构、后台治理、管理前端、聊天前端
- 进行中重点：查询预处理管道扩展（语义丰富化+脱敏过滤）、语义/父子分块、Cross-Encoder 重排序、Dashboard 扩展、分块策略 YAML 参数覆写
